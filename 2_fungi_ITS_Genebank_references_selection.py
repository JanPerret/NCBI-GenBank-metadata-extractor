...
def select_genbank_references(filename, outf):
	...
	for line in fin:
		...
		if keyword == "REFERENCE ":
			parsing_organism = False
			try:
				ind = organism.index("Fungi") # taxa condition
			except:
				continue
			if "internal transcribed spacer" in definition.lower() or "ITS" in definition: # gene condition
				to_be_written = True
			continue
		if parsing_organism:
...
import glob
outf = open('references_fungi_ITS.txt','w') # create output file where selected references will be written
