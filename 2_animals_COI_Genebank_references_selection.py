	...
	for line in fin:
		...
		if keyword == "REFERENCE ":
			...
			try:
				...
				continue
			if "cytochrome oxidase 1" in definition.lower() or "cytochrome oxidase i" in definition.lower() or "COI" in definition or "co1" in definition.lower(): # gene condition
				to_be_written = True
			continue
		if parsing_organism:
...
import glob
outf = open('references_animals_COI.txt','w') # create output file where selected references will be written
