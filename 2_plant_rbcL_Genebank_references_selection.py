	...
	for line in fin:
		...
		if keyword == "REFERENCE ":
			...
			try:
				...
				continue
			if "ribulose bisphosphate carboxylase" in definition.lower() or "rbcl" in definition.lower(): # gene condition
				to_be_written = True
			elif "ribulose" in definition.lower() and "bisphosphate" in definition.lower() and "carboxylase" in definition.lower() : # 2nd gene condition (because this gene name if often in multiple parts)
				to_be_written = True
			continue
		if parsing_organism:
...
import glob
outf = open('references_plants_rbcL.txt','w') # create output file where selected references will be written
